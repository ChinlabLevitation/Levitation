...
plate_temp = 301  # bottom plate (hot plate)
boundary_temp = 289  # boundary temperature (room temperature)

# geometry of the system in inches
bucket_radius = 0.7  # radius of the top plate
plate_radius = 1  # radius of the bottom plate
plate_height = 1  # height of the bottom plate
separation_height = 1  # separation between the top bucket and the bottom plate
simulation_radius = 2  # radius of the simulation
simulation_height = 3  # height of the simulation

# define the number of grid points
Nr = 200  # number of grid points in the radial direction
Nz = 200  # number of grid points in the axial direction
...
F = -f_T(averagePressure, fullT, particleRadius) * particleRadius**2 * kair(fullT)/np.sqrt(2 * boltzmannConstant * fullT/molecMassAir) * grad_T
F_mag = np.sqrt(F[0] * F[0] + F[1] * F[1])
